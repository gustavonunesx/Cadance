...
| Geração do cronograma | O motor distribui as tarefas em blocos diários respeitando: disponibilidade, prazo, prioridade e folga de segurança. O preenchimento é cronológico. (Bloco de pico — tarefa pesada no horário mais produtivo — fica para a v2; ver `scheduler-spec.md` §9.) |
...
create table tasks (
  ...
  needs_clarification boolean default false,  -- IA não conseguiu estimar; não entra no cronograma
  created_at timestamptz default now()
);
...
**4. Tarefa pesada no bloco de pico — v2, não MVP.** Se o usuário marcou um
período como "pico" (mais produtivo), o algoritmo prioriza colocar ali as
tarefas mais longas ou difíceis. **Adiado para a v2** (`scheduler-spec.md` §9):
fazer isso sem arriscar prazo compete com o earliest-deadline-first e exige
regras extras difíceis de testar. No MVP o preenchimento é cronológico e
`isPeak`/`difficulty` são ignorados — o usuário ainda marca o bloco de pico, o
dado é guardado, só não influencia a distribuição ainda.
...
A IA **não extrai tarefas** — quem faz isso é `src/lib/md/`, com parse
determinístico do checklist (cada `- [ ]` vira uma tarefa, na ordem do
arquivo). A IA recebe a lista já pronta e faz só a parte que é julgamento:
estimar duração. Ver `docs/superpowers/specs/2026-09-16-cadence-pipeline-ia-design.md` §3.

**System prompt — estimativa de duração:**
```
Você recebe o contexto de um projeto e uma lista de tarefas já extraídas do
checklist. Sua função é APENAS estimar a duração de cada tarefa — você NÃO
monta cronograma, NÃO soma horas e NÃO cria, remove ou renomeia tarefas
(isso é feito por outro sistema). Responda em pt-BR.

Retorne um JSON: uma lista com um objeto por tarefa recebida, usando
exatamente os mesmos "id" que chegaram. Para cada uma:
- "id": o id recebido, inalterado
- "estimated_minutes": estimativa realista em minutos (inteiro, 1 a 480)
- "difficulty": "leve" | "media" | "pesada"
- "needs_clarification": true se o contexto não permite estimar

Regras de estimativa:
- Seja realista, não otimista. É melhor superestimar um pouco.
- Se uma tarefa parece passar de 480 min (8h), ela está grande demais para
  uma estimativa confiável: marque "needs_clarification": true.
- Se o contexto não der detalhe suficiente pra estimar, marque
  "needs_clarification": true naquela tarefa. Não chute.
...
> A saída passa por validação (Zod) antes de qualquer uso: `estimated_minutes`
> fora da faixa `1..480`, ausente, ou com tipo errado vira
> `needs_clarification: true` naquela tarefa. JSON malformado → 1 retry → se
> falhar de novo, todas as tarefas nascem flagueadas e o usuário estima na mão.
> O import nunca falha por completo.
>
> Tarefa com `needs_clarification` **não entra no cronograma** — aparece em
> `unestimatedTasks` com um alerta próprio. O plano gerado é real; só não está
> completo. A IA nunca vê a agenda — ela só entende o projeto.
